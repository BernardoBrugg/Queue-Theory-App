...
export function ChronometerCard({
  ...
  const typeColor =
    queueType === "arrival" ? "var(--accent-cyan)" : "var(--accent)";
  const statusLabel = isService ? (isOccupied ? "OCUPADO" : "LIVRE") : "ATIVO";
  const statusColor = isService
    ? isOccupied
      ? "var(--danger)"
      : "var(--success)"
    : "var(--accent-cyan)";
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === keyboardShortcut.toLowerCase()) {
        if (queueType === "arrival") {
          ...
          if (!isOccupied) arrivedAtService();
          else completedService();
        }
      }
    };
          ...
          >
            ...
            {keyboardShortcut && (
              <div
                ...
                title={`Atalho de teclado: tecla ${keyboardShortcut.toUpperCase()}`}
              >
        ...
        {isService && (
          ...
          >
            <div
              style={{
                fontSize: "0.7rem",
                color: isOccupied ? "var(--accent)" : "var(--text-muted)",
                fontWeight: 600,
                textTransform: "uppercase",
                marginBottom: "0.25rem",
              }}
            ...
            <div
              style={{
                fontWeight: 800,
                fontSize: "1.25rem",
                color: isOccupied ? "var(--accent)" : "var(--text-primary)",
              }}
            >
              {isOccupied ? "SIM" : "NÃO"}
            </div>
